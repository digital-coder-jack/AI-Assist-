...
const { config, chunk, archiveRecord, parseArchiveRecord, memberRecordText, sessionRecordText, conversationRecordText, statsText, SCHEMA_VERSION } = require('../src/backend/telegram-archive');
...
test('Telegram records are deterministic, versioned, and identity-scoped', () => {
  const event = { userId: '123', username: 'jack', displayName: 'Jack', guildId: '456', channelId: '789', sessionId: 'session:dm:123', mode: 'private_dm', timestamp: '2026-08-29T00:00:00Z', question: 'How do I deploy?', response: 'Use the deployment checklist.', language: 'English', provider: 'groq', eventId: 'm1:success', attachments: [{ id: 'a1', filename: 'guide.pdf' }] };
  ...
  for (const text of [member, session, conversation, stats]) { assert.match(text, /\[FORGE_ASSIST\]/); assert.match(text, new RegExp(`schema_version=${SCHEMA_VERSION}`)); assert.match(text, /discord_user_id="123"/); assert.doesNotMatch(text, /DISCORD_TOKEN|API_KEY|TELEGRAM_BOT_TOKEN/); }
  assert.match(member, /record_type=MEMBER_CREATED/);
  assert.match(session, /session_id="session:dm:123"/);
  assert.match(conversation, /record_type=CONVERSATION_RECORD/);
  assert.match(conversation, /attachment_ids=\["a1"\]/);
  assert.match(stats, /record_type=EVENT_RECORD/);
});
...
test('Telegram archive writes member, session, conversation, and event records to the configured destination', async () => {
  ...
  assert.match(text, /record_type=MEMBER_CREATED/);
  assert.match(text, /record_type=SESSION_STARTED/);
  assert.match(text, /record_type=CONVERSATION_RECORD/);
  assert.match(text, /record_type=EVENT_RECORD/);

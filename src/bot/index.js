...
function communityContext(message) {
  const guild = message.guild;
  const roles = guild?.roles?.cache ? [...guild.roles.cache.values()].filter(role => !role.managed && role.name !== '@everyone').slice(0, 30).map(role => role.name) : [];
...
function registerMessageHandlers(client, { logger = console, env = process.env } = {}) { client.on('messageCreate', message => handleMessage(message, client, { logger, env })); return registerOnboardingHandlers(client, { postArchive, env, logger }); }
